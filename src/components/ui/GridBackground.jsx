"use client"
import React from "react";

export function GridBackground() {
    return (
        <div
            className="h-[50rem] w-full dark:bg-black-100 bg-white  dark:bg-grid-white/[0.035] bg-grid-black/[0.2] absolute left-0 top-0 flex md:items-center justify-center">
            <div
                className="absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black-100 bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]" />
        </div>
    );
}
